...
if isinstance(df_main, pd.DataFrame):
    ...
    with tab_search:
        ...
        # 搜尋欄位 (透過關鍵字觸發 force_smart_keyboard 邏輯)
        search_name = st.text_input("📝 品名關鍵字", placeholder="輸入品名搜尋...") # 預設鍵盤
        search_code = st.text_input("🔢 條碼/代號搜尋", placeholder="點擊輸入數字條碼...") # 數字鍵盤
        ...
        if has_search:
            ...
            if not work_df.empty:
                ...
                for _, row in work_df.iterrows():
                    ...
                    with st.container(border=True): # 手機版卡片式排版
    ...
    with tab_add:
        ...
        new_bc = st.text_input("商品條碼 (必填)", placeholder="輸入條碼數字") # 這邊也會觸發數字鍵盤
